...
const newInvoice = () => billing.createInvoice({
  patient: patient._id,
  items: [{ description: 'Procedure', quantity: 1, unitPrice: 1000 }],
}, null);
...
describe('invoice payment concurrency', () => {
  it('never lets concurrent payments exceed the grand total', () => inTenant(async () => {
    ...
    const results = await Promise.allSettled([1, 2, 3].map(() =>
      billing.recordPayment(inv._id, { amount: 1000, method: 'CASH' }, null)
    ));
    ...
    expect(await ledgerBalance(inv._id)).toBe(1000); // exactly one receipt
  }));
  ...
  it('adds up concurrent partial payments without losing any', () => inTenant(async () => {
    ...
    const results = await Promise.allSettled([1, 2, 3, 4].map(() =>
      billing.recordPayment(inv._id, { amount: 250, method: 'CASH' }, null)
    ));
    ...
    expect(after.paidAmount).toBe(1000);
    expect(after.status).toBe('PAID');
    expect(await ledgerBalance(inv._id)).toBe(1000);
  }));

  it('keeps status and dueAmount consistent on a partial payment', () => inTenant(async () => {
    const inv = await newInvoice();
    const { invoice } = await billing.recordPayment(inv._id, { amount: 400, method: 'CARD' }, null);

    expect(invoice.paidAmount).toBe(400);
    expect(invoice.dueAmount).toBe(600);
    expect(invoice.status).toBe('PARTIAL');
  }));

  it('never refunds more than was actually paid', () => inTenant(async () => {
    const inv = await newInvoice();
    await billing.recordPayment(inv._id, { amount: 600, method: 'CASH' }, null);

    // Two clerks each try to refund the whole ₹600.
    const results = await Promise.allSettled([1, 2].map(() =>
      billing.refundInvoice(inv._id, { amount: 600, method: 'CASH', reason: 'Cancelled procedure' }, null)
    ));
  ...
  it('leaves a partial refund re-derivable as PARTIAL', () => inTenant(async () => {
    const inv = await newInvoice();
    await billing.recordPayment(inv._id, { amount: 1000, method: 'CASH' }, null);
    const { invoice } = await billing.refundInvoice(inv._id, { amount: 300, method: 'CASH' }, null);

    expect(invoice.paidAmount).toBe(700);
    expect(invoice.dueAmount).toBe(300);
    expect(invoice.status).toBe('PARTIAL');
    expect(await ledgerBalance(inv._id)).toBe(700);
  }));
  ...
  it('banks a gateway payment once even if verify and webhook both fire', () => inTenant(async () => {
    ...
    const results = await Promise.allSettled([
      billing.recordPayment(inv._id, { amount: 1000, method: 'ONLINE', transactionId: txn }, null),
      billing.recordPayment(inv._id, { amount: 1000, method: 'ONLINE', transactionId: txn }, null),
    ]);
  ...
  it('refuses to bank the same gateway transaction id twice', () => inTenant(async () => {
    ...
    await billing.recordPayment(inv._id, { amount: 400, method: 'ONLINE', transactionId: txn }, null);
    await expect(billing.recordPayment(inv._id, { amount: 400, method: 'ONLINE', transactionId: txn }, null))
    ...
    expect(after.dueAmount).toBe(600);
    expect(after.status).toBe('PARTIAL');
    expect(await ledgerBalance(inv._id)).toBe(400);
  }));

  it('still allows several desk payments with no transaction id', () => inTenant(async () => {
    const inv = await newInvoice();
    await billing.recordPayment(inv._id, { amount: 300, method: 'CASH' }, null);
    await billing.recordPayment(inv._id, { amount: 300, method: 'CASH' }, null);

    expect((await Invoice.findById(inv._id)).paidAmount).toBe(600);
  ...
  it('rejects payment on a cancelled invoice', () => inTenant(async () => {
    ...
    await expect(billing.recordPayment(inv._id, { amount: 100, method: 'CASH' }, null))
