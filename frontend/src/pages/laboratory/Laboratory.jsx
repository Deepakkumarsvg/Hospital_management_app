...
import { FlaskConical, Plus, X, Search, Download, Clock, ShieldCheck } from 'lucide-react';
...
import { listLabOrders, exportLabOrders, getLabStats } from '../../services/labService.js';
...
function Orders() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try { setData(await listLabOrders({ page, limit: 20, search, status, doctor: doctor || undefined, patient: patientFilter || undefined })); }
    catch (err) { toast.error(err.message || 'Failed'); } finally { setLoading(false); }
  }, [page, search, status, doctor, patientFilter, toast]);
  useEffect(() => { fetchData(); }, [fetchData]);
  useEffect(() => {
    getLabStats().then(setStats).catch(() => {});
  ...
  const onExport = async (format) => {
    setExporting(format);
    try { await exportLabOrders({ search, status, doctor: doctor || undefined, patient: patientFilter }, format); }
    ...
    <div className="space-y-4">
      {stats && (
        <div className="grid grid-cols-3 gap-4">
          <StatTile label="Total Orders" value={stats.total} icon={FlaskConical} />
          <StatTile label="Pending" value={stats.pending} tone="text-amber-600 dark:text-amber-400" icon={Clock} />
          <StatTile label="Verified" value={stats.verified} tone="text-green-600 dark:text-green-400" icon={ShieldCheck} />
        </div>
      )}
